...
import brightspark.runicmagic.entity.render.RenderSpellProjectile;
...
public class RMEntities
{
	...
	private static EntityEntryBuilder<Entity> createBuilder(Class<? extends Entity> entityClass, String name, int updateFrequency, boolean sendVelocityUpdates)
	{
		return EntityEntryBuilder.create()
			.entity(entityClass)
			.name(String.format("%s.%s", RunicMagic.MOD_ID, name))
			.id(new ResourceLocation(RunicMagic.MOD_ID, name), entityId++)
